...
{
    NVIMGCDCS_LOG_TRACE_("Codec::getParser " << name_);
    for (const auto& entry : parsers_) {
        NVIMGCDCS_LOG_TRACE_("- probing parser:" << entry.second->getParserId());
        if (entry.second->canParse(code_stream)) {
            NVIMGCDCS_LOG_TRACE_("- - can parse");
