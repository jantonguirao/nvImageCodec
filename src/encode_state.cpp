...
EncodeState::EncodeState(const nvimgcdcsEncoderDesc_t encoder_desc,
    nvimgcdcsEncoder_t encoder)
    : encoder_desc_(encoder_desc)
    , encoder_(encoder)
{
    encoder_desc_->createEncodeState(encoder_, &encode_state_);
}
...
void EncodeState::setPromise(const ProcessingResultsPromise& promise)
{
    promise_ = std::make_unique<ProcessingResultsPromise>(promise);
}

const ProcessingResultsPromise& EncodeState::getPromise()
{
    return *promise_;
}
