...
NvJpegHwDecoderPlugin::Decoder::Decoder(const char* plugin_id, const nvimgcdcsFrameworkDesc_t* framework, int device_id,
    ...
    nvimgcdcsDecoder_t* decoder, int device_id, const nvimgcdcsBackendParams_t* backend_params, const char* options)
{
    try {
        ...
        *decoder = reinterpret_cast<nvimgcdcsDecoder_t>(
            new NvJpegHwDecoderPlugin::Decoder(plugin_id_, framework_, device_id, backend_params, options));
    } catch (const NvJpegException& e) {
