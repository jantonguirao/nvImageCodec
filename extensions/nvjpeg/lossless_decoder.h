...
class NvJpegLosslessDecoderPlugin
{
  ...
  private:
    ...
    struct Decoder
    {
        Decoder(const char* plugin_id, const nvimgcdcsFrameworkDesc_t* framework, int device_id,
            const nvimgcdcsBackendParams_t* backend_params, const char* options = nullptr);
        ~Decoder();
