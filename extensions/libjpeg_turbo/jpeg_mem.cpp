...
std::unique_ptr<uint8_t[]> UncompressLow(const void* srcdata, FewerArgsForCompiler* argball)
{
    ...
    auto sample_format = flags.sample_format;
    ...
    auto destroy_cinfo = AtScopeExit([&cinfo]() { jpeg_destroy_decompress(&cinfo); });
    ...
    int64_t total_size =
        static_cast<int64_t>(cinfo.output_height) * static_cast<int64_t>(cinfo.output_width) * static_cast<int64_t>(cinfo.num_components);
    ...
    if (flags.crop) {
        ...
        if (skipped_scanlines != static_cast<JDIMENSION>(flags.crop_y))
    ...
    const int mcu_align_offset = (cinfo.output_width - target_output_width - right_cor) * (use_cmyk ? 4 : components);
    while (cinfo.output_scanline < static_cast<JDIMENSION>(max_scanlines_to_read)) {
        ...
        if (num_lines_read != 1)
            throw std::runtime_error("Unexpected number of lines read");
        output_line += stride;
    }
    temp.reset();
    tempdata = nullptr;
    ...
    if (flags.crop && cinfo.output_scanline < cinfo.output_height) {
        ...
        if (cinfo.output_height != cinfo.output_scanline)
            throw std::runtime_error("Unexpected output scan line");
    }
    ...
    if (components == 4) {
        ...
        JSAMPLE* scanlineptr = static_cast<JSAMPLE*>(dstdata.get() + static_cast<int64_t>(target_output_height - 1) * stride);
...
//  it out a little.
std::unique_ptr<uint8_t[]> Uncompress(const void* srcdata, int datasize, const UncompressFlags& flags)
{
