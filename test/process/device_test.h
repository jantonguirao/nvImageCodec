...
#define DEV_CHECK_OP_AB(a, b, op, is_fatal) { \
  ...
  if (!(a_result op b_result)) { \
    auto a_str = nvimgcodec::dev_to_string(a_result); \
    auto b_str = nvimgcodec::dev_to_string(b_result); \
...
__global__ void suite_name##_##test_name##_kernel( \
    nvimgcodec::testing::TestStatusBlock *test_status, Args... args)

#define DEFINE_TEST_KERNEL(suite_name, test_name, ...) \
__device__ void suite_name##_##test_name##_body( \
    nvimgcodec::testing::TestStatusBlock &test_status, ##__VA_ARGS__); \
\
template <typename... Args>\
__global__ void suite_name##_##test_name##_kernel(\
      nvimgcodec::testing::TestStatusBlock *test_status, Args... args) { \
...
#define DEVICE_TEST_CASE_BODY(suite_name, test_name, grid, block, ...)                  \
  nvimgcodec::testing::TestStatus status;                                                     \
  if (!status.Init())                                                                   \
    return;                                                                             \
  nvimgcodec::testing::ClearCudaError();                                                      \
