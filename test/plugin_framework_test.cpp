...
TEST(PluginFrameworkTest, test_ext_module_discovery)
{
    ...
    std::unique_ptr<MockPostprocessor> postprocessor = std::make_unique<MockPostprocessor>();

    PluginFramework framework(&codec_registry, std::move(directory_scaner),
        std::move(library_loader), std::move(executor), std::move(postprocessor), nullptr, nullptr);
